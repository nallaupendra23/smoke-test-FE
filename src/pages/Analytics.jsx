...
const EXPORT_SECTIONS = [
  { key: 'summary',    label: 'Order summary & revenue' },
  { key: 'calls',      label: 'AI call analytics' },
  { key: 'callVolume', label: 'Daily call volume table' },
  { key: 'revenue',    label: 'Revenue by date' },
  { key: 'orders',     label: 'Full order history' },
  { key: 'payments',   label: 'Payment status breakdown' },
]
const DEFAULT_EXPORT_SECTIONS = EXPORT_SECTIONS.reduce((acc, section) => {
  acc[section.key] = true
  return acc
}, {})

function makeLocalReport({ period, callData, statsData }) {
  ...
  const revenue = orders
    .filter(order => order.status !== 'cancelled' && (order.payment_status === 'paid' || ['cash', 'card_on_pickup'].includes(order.pay_method)))
    .reduce((sum, order) => sum + Number(order.total || 0), 0)

  const revenueByDate = orders.reduce((acc, order) => {
    if (!order.created_at || order.status === 'cancelled') return acc
    const key = order.created_at.slice(0, 10)
    if (!acc[key]) acc[key] = 0
    if (order.payment_status === 'paid' || ['cash', 'card_on_pickup'].includes(order.pay_method)) {
      acc[key] += Number(order.total || 0)
    }
    return acc
  }, {})

  return {
    restaurant_name: 'RingZ.ai',
    period,
    ...
    summary: {
      ...
      avg_order_value: completedOrders.length ? Number((revenue / completedOrders.length).toFixed(2)) : 0,
    },
    calls: {
      total: callData?.total_calls || 0,
      completed: callData?.completed_calls || 0,
      abandoned: callData?.abandoned_calls || 0,
      completion_rate: callData?.completion_rate || 0,
      avg_duration_seconds: callData?.avg_duration_seconds || 0,
    },
    calls_by_date: (callData?.calls_by_date || []).map(row => ({
      date: row.date,
      completed: row.completed ?? row.count ?? 0,
      abandoned: row.abandoned ?? 0,
    })),
    revenue_by_date: Object.entries(revenueByDate)
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([date, value]) => ({ date, revenue: Number(value.toFixed(2)) })),
    orders: orders.map(order => ({
      ...
      items: Array.isArray(order.items)
        ? order.items.map(item => `${item.quantity || 1}x ${item.name}${item.modification ? ` (${item.modification})` : ''}`).join(', ')
        : '—',
    })),
  }
}
...
/* ── Small metric card ── */
function MetricCard({ label, value, sub, color, progress, style }) {
  return (
...
export default function Analytics() {
  ...
  const [exportSections, setExportSections] = useState(DEFAULT_EXPORT_SECTIONS)
  ...
  const handleDownload = async () => {
    if (!Object.values(exportSections).some(Boolean)) {
      alert('Choose at least one report section to export.')
      return
    }
    setDownloading(true)
    setDlSuccess(false)
    try {
      ...
      try {
        ...
        console.warn('Report endpoint failed; generating PDF from loaded analytics data:', apiErr)
        reportData = makeLocalReport({ period: dlPeriod, callData, statsData })
      }

      await generateReportPDF(reportData, { sections: exportSections })
      setDlSuccess(true)
  ...
  return (
    <Layout>
      <div className="app-page">
        ...
        <PageHeader
          icon={AnalyticsIcon}
          eyebrow="AI Voice Analytics"
          title="Performance Analytics"
          subtitle={lastUpdated ? `Live data · updated ${lastUpdated.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}` : 'Live call and order trends'}
          ...
          <div className="page-toolbar">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: calendarDate ? 'rgba(170,48,26,0.08)' : 'var(--surface-3)',
                  border: calendarDate ? '1.5px solid rgba(170,48,26,0.30)' : '1.5px solid transparent',
                  fontSize: 12, fontWeight: 600,
                  color: calendarDate ? '#aa301a' : 'var(--text-3)',
            ...
            <button
              ...
              style={{
                ...
                boxShadow: '0 2px 8px rgba(170,48,26,0.30)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
          ...
          <div className="card" style={{ padding: '24px 24px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              <div style={{ width: 40, height: 40, borderRadius: 12, background: '#fff2ee', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <PhoneCallIcon size={17} style={{ color: '#aa301a' }} />
              </div>
                  ...
                  return (
                    <button key={f.label} onClick={() => { setCardCalendarDate(''); setCardFilter({ mode: 'days', value: f.value }) }}
                      style={{
                        ...
                        color: active ? '#aa301a' : 'var(--text-3)',
          ...
          <div className="card" style={{ padding: '24px 24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              <div style={{ width: 40, height: 40, borderRadius: 12, background: '#fff2ee', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <WalkInIcon size={17} style={{ color: '#aa301a' }} />
              </div>
              <div>
            ...
            {loading ? (
              ...
              <div style={{ marginTop: 4 }}>
                <ProgressRow label="Phone Orders"     value={phoneOrders}   total={totalOrders} color="#aa301a" textColor="#aa301a" />
                <ProgressRow label="Walk-in Orders"   value={walkInOrders}  total={totalOrders} color="#9a2b17" textColor="#9a2b17" />
            ...
            {!loading && statsData && (
              <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontSize: 20, fontWeight: 700, color: '#aa301a', fontFamily: 'Noto Serif, serif' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20, alignItems: 'stretch' }}>
          ...
          <div className="card" style={{
            ...
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(184,66,38,0.18) 0%, rgba(8,21,37,0) 48%, rgba(47,128,237,0.12) 100%)', pointerEvents: 'none' }} />

            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(170,48,26,0.40)', border: '1px solid rgba(170,48,26,0.50)', borderRadius: 99, padding: '4px 12px', marginBottom: 16 }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#cb4830', boxShadow: '0 0 6px #cb4830', display: 'inline-block' }} />
                ...
                {PERIODS.map(p => (
                  <button
                    ...
                    style={{
                      ...
                      background:   dlPeriod === p.value ? '#aa301a'  : '#fff',
                      color:        dlPeriod === p.value ? '#fff'     : '#475569',
                      borderColor:  dlPeriod === p.value ? '#aa301a'  : '#c2d4ef',
                      boxShadow:    dlPeriod === p.value ? '0 2px 8px rgba(170,48,26,0.25)' : 'none',
                    }}
                  >
                    {p.label}
                  </button>
                    ...
                    return (
                      <label
                        ...
                        style={{
                          ...
                          padding: '7px 10px',
                          borderRadius: 10,
                          border: `1px solid ${active ? 'rgba(170,48,26,0.28)' : '#c2d4ef'}`,
                          background: active ? 'rgba(170,48,26,0.07)' : '#fff',
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={active}
                          onChange={() => setExportSections(prev => ({ ...prev, [section.key]: !prev[section.key] }))}
                          style={{ accentColor: '#aa301a', width: 14, height: 14, flexShrink: 0 }}
                        />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, flexShrink: 0, paddingTop: 4 }}>
              <button
                ...
                style={{
                  ...
                  fontSize: 14,
                  minWidth: 160,
                  opacity: downloading || !Object.values(exportSections).some(Boolean) ? 0.62 : 1,
                  cursor: downloading || !Object.values(exportSections).some(Boolean) ? 'not-allowed' : 'pointer',
                }}
              >
                {downloading ? (
              ...
              <span style={{ fontSize: 11, color: '#aa301a', fontWeight: 500 }}>
                {PERIODS.find(p => p.value === dlPeriod)?.label} · {Object.values(exportSections).filter(Boolean).length} sections
              </span>
            </div>
          </div>
        </div>
